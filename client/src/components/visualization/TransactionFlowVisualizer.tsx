    ...
    onMessage: (event) => {
      try {
        ...
        if (data.type === 'transaction' || data.type === 'new_transaction') {
          ...
          const transaction = {
            ...data.transaction || data.data, // Handle both formats
            timestamp: new Date(data.transaction?.timestamp || data.data?.timestamp || data.timestamp),
            id: (data.transaction?.hash || data.data?.hash || crypto.randomUUID()) // Use hash as ID if not provided
          };
